...
It is aimed to apply some of the tools for potato-brained (by this I mean boring and time-consuming, no insult to actual potato-owners) RE tasks sequentially until user is satisfied (for now it tries to rename variables, list ascii-readable string constants and shoould rename functions based on its functionality (but doesn't work yet), but potentialy it's extendable to any number of tasks (some of which may be implemented in parallel as long as you have enough VRAM), if you can find the right system prompt, that's it;
